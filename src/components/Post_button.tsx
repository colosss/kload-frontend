...
export default function Post_button(props: Post_button_props) {
  ...
  function Button_click() {
    ...
    // Исправлено: безопасный вызов, если setfunc передан
  ...
  return (
    <div>
      ...
      <button className='post_button' type="button" onClick={() => Button_click()}>
        <img 
          className='post_image_button' 
          src={!button_flag ? (put ? red_defualte : defualte) : cancel} 
          alt={!button_flag ? 'post_button' : 'post_button_cancel'}
        />
      </button>
      {button_flag && 
